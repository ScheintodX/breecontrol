...
//const char *ssid =  "CASTLE";    // cannot be longer than 32 characters!
//const char *password =  "9453947466983375";    //
const char *ssid =  "TP-LINK_Axone";    // cannot be longer than 32 characters!
const char *password =  "l2h6XdxQuDWB8QIa";    //
const char* mqtt_server = "10.3.2.1";
//const char* mqtt_server = "hass.fritz.box";
const char* mqtt_username = "test";
const char* mqtt_password = "test";
//const char* mqtt_username = "client";
//const char* mqtt_password = "Wv6hKB3bFFasI";
...
int hex2int( String s, int index ) {
    return (b2int( s.charAt(index) )<<4) + b2int(s.charAt(index+1));;
}
  ...
  if(String(topic) == (BASE+"upper/temp/set")){
   ...
   if ( payload_len != PIXELS * 3 * 2 ) return;
    ...
    for( int i = 0; i < PIXELS; i++ ) {
      ...
      colorsIn[ i ].R = hex2int( s_pl, i*3*2 + 0*2 );
      colorsIn[ i ].G = hex2int( s_pl, i*3*2 + 1*2 );
      colorsIn[ i ].B = hex2int( s_pl, i*3*2 + 2*2 );
    }
